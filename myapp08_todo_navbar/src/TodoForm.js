...
const TodoForm= (props)=>{//{속성값이름}
    ...
    const submitTodo= ()=>{ //서버에 입력된 데이터의 값을 보낸다(TodoTable에 데이터 추가)
        ...
        //'http://localhost:8875/todo/insert' -> 크로스도메인 사용
        fetch('http://localhost:8875/todo/insert',{ //json형식
            method:'post', 
            ...
            alert("등록완료")
            props.history.push("/")//등록완료후 리다이렉트(history.push)기능을 사용하여 Home.js로 이동되도록 설정
        })
